 ...
 ** SQLITE_SOURCE_ID "2024-03-07 12:34:26 720ce06d93a9e4cc25c34c873c82165d8801f208c22701e51538f3210de8alt1"
 **
 ** Using the Emscripten SDK version emcc (Emscripten gcc/clang-like replacement + linker emulating GNU ld) 3.1.47-git.
 */
...
var sqlite3InitModule = (() => {
  ...
  return function (moduleArg = {}) {
    ...
    /**
     * This custom locateFile() tries to figure out where to load `path` from.
     * The intent is to provide a way for foo/bar/X.js loaded from a Worker
     * constructor or importScripts() to be able to resolve foo/bar/X.wasm (in
     * the latter case, with some help):
     *
     * 1. If URL param named the same as `path` is set, it is returned.
     * 2. If sqlite3InitModuleState.sqlite3Dir is set, then (thatName + path) is
     *    returned (note that it's assumed to end with '/').
     * 3. If this code is running in the main UI thread AND it was loaded from a
     *    SCRIPT tag, the directory part of that URL is used as the prefix.
     *    (This form of resolution unfortunately does not function for scripts
    ...
    /**
     * Bug warning: a custom Module.instantiateWasm() does not work in WASMFS
     * builds:
     *
...
/** not-t@type {!BigUint64Array} */
     ...
     *   middle, then the string will cut short at that byte index (i.e.
     *   maxBytesToRead will not produce a string of exact length [ptr,
     *   ptr+maxBytesToRead[) N.B. mixing frequent uses of UTF8ToString() with
     *   and without maxBytesToRead may throw JS JIT optimizations off, so it is
     *   worth to consider consistently using one
     * @returns {string}
     */
    ...
    var FSNode = /** @class */ function (parent, name, mode, rdev) {
      if (!parent) {
    ...
    // include: /build/source/ext/wasm/bld/post-js.sqlite3-vanilla.js
    /* BEGIN FILE: api/post-js-header.js */
    /**
     * Post-js-header.js is to be prepended to other code to create post-js.js
     * for use with Emscripten's --post-js flag. This code requires that it be
     * running in that context. The Emscripten environment must have been set up
     * already but it will not have loaded its WASM when the code in this file
     * is run. The function it installs will be run after the WASM module is
     * loaded, at which point the sqlite3 JS API bits will get set up.
     */
    if (!Module.postRun) Module.postRun = [];
    Module.postRun.push(
      function (Module /*the Emscripten-style module object*/) {
        'use strict';
...
*/
        ...
        /**
         * Sqlite3ApiBootstrap() is the only global symbol persistently exposed
         * by this API. It is intended to be called one time at the end of the
         * API amalgamation process, passed configuration details for the
         * current environment, and then optionally be removed from the global
         * object using `delete globalThis.sqlite3ApiBootstrap`.
         *
         ...
         * that it can be used without any direct dependency on Emscripten.
         * (Note the default values for the config object!) The config object is
         * only honored the first time this is called. Subsequent calls ignore
         * the argument and return the same (configured) object which gets
         * initialized by the first call. This function will throw if any of the
         * required config options are missing.
         *
         * The config object properties include:
         *
         * - `exports`[^1]: the "exports" object for the current WASM environment.
         *   In an Emscripten-based build, this should be set to
         *   `Module['asm']`.
         * - `memory`[^1]: optional WebAssembly.Memory object, defaulting to
         *   `exports.memory`. In Emscripten environments this should be set to
         *   `Module.wasmMemory` if the build uses `-sIMPORTED_MEMORY`, or be
         *   left undefined/falsy to default to `exports.memory` when using
         *   WASM-exported memory.
         * - `bigIntEnabled`: true if BigInt support is enabled. Defaults to true
         *   if `globalThis.BigInt64Array` is available, else false. Some APIs
         ...
         *   allocators under very specific conditions. The canonical builds of
         *   this API guaranty that `sqlite3_malloc()` is the JS-side allocator
         *   implementation.
         * - `deallocExportName`: the name of the function, in `exports`, of the
         *   `free(3)`-compatible routine for the WASM environment. Defaults to
         *   `"sqlite3_free"`.
         * - `reallocExportName`: the name of the function, in `exports`, of the
         *   `realloc(3)`-compatible routine for the WASM environment. Defaults
         *   to `"sqlite3_realloc"`.
         * - `debug`, `log`, `warn`, and `error` may be functions equivalent to
         *   the like-named methods of the global `console` object. By default,
         *   these map directly to their `console` counterparts, but can be
         *   replaced with (e.g.) empty functions to squelch all such output.
         * - `wasmfsOpfsDir`[^1]: Specifies the "mount point" of the OPFS-backed
         *   filesystem in WASMFS-capable builds.
         *
         * [^1] = This property may optionally be a function, in which case this
         * function calls that function to fetch the value, enabling delayed
         * evaluation.
         *
         ...
         * default config value. The intention of this is to provide downstream
         * clients with a reasonably flexible approach for plugging in an
         * environment-suitable configuration without having to define a new
         * global-scope symbol.
         *
         ...
         * define globalThis.sqlite3ApiConfig to an object. If it is set, it is
         * used instead of sqlite3ApiBootstrap.defaultConfig if
        ...
        */
          ...
          /**
           * Eliminate any confusion about whether these config objects may be
           * used after library initialization by eliminating the outward-facing
           * objects...
           */
           ...
           * possible, identically to the C-native counterparts, as documented
           * at:
           *
          ...
          /**
           * Holds state which are specific to the WASM-related infrastructure
           * and glue code.
           *
           ...
           * enabling clients to unambiguously identify such exceptions. The
           * C-level APIs never throw, but some of the higher-level C-style APIs
           * do and the object-oriented APIs use exceptions exclusively to
           * report errors.
           */
          ...
          /**
           * Functionally equivalent to the SQLite3Error constructor but may be
           * used as part of an expression, e.g.:
           *
           *     ```
          ...
          /**
           * Returns true if n is a 32-bit (signed) integer, else false. This is
           * used for determining when we need to switch to double-type DB
           * operations for integer values in order to keep more precision.
           */
          const isInt32 = (n) => {
            return (
              'bigint' !==
          ...
          /**
           * Returns true if the given BigInt value is small enough to fit into
           * an int64 value, else false.
           */
          ...
          const bigIntFits32 = (b) =>
            b >= -0x7fffffffn - 1n && b <= 0x7fffffffn;

          /**
           * Returns true if the given BigInt value is small enough to fit into
           * a double value without loss of precision, else false.
           */
           ...
           * between TypedArrays which are backed by a SharedArrayBuffer from
           * those which are not.
           */
          const __SAB =
          ...
          /**
           * Returns either aTypedArray.slice(begin,end) (if aTypedArray.buffer
           * is a SharedArrayBuffer) or aTypedArray.subarray(begin,end) (if it's
           * not).
           *
           ...
           * TypedArrays with element sizes >1 is a potential TODO just waiting
           * on a use case to justify them. Until then, their `buffer` property
           * can be used to pass them as an ArrayBuffer. If it's not a bindable
           * array type, a falsy value is returned.
           */
          ...
          /**
           * Returns true if v appears to be one of the TypedArray types which
           * is legal for holding SQL code (as opposed to binary blobs).
           *
          ...
          /**
           * Returns true if isBindableTypedArray(v) does, else throws with a
           * message that v is not a supported TypedArray value.
           */
           ...
           * backed by a SharedArrayBuffer or not, and can work more efficiently
           * if it's not (TextDecoder refuses to act upon an SAB).
           */
          ...
          class WasmAllocError extends Error {
             ...
             * arguments together with a single space between each to construct
             * an error message string. As a special case, if called with no
             * arguments then it uses a default error message.
             */
            constructor(...args) {
              if (2 === args.length && 'object' === typeof args[1]) {
                super(...args);
          ...
          Object.assign(
            capi,
            {
              ...
              /**
               * Equivalent to calling sqlite3_prapare_v3() with 0 as its 4th
               * argument.
               */
              sqlite3_prepare_v2: (
                dbPtr,
          ...
          /**
           * Various internal-use utilities are added here as needed. They are
           * bound to an object only so that we have access to them in the
           * differently-scoped steps of the API bootstrapping process. At the
           * end of the API setup process, this object gets removed. These are
          ...
          Object.assign(
            wasm,
            {
               ...
               * pointer-type values. It MUST refer to a value type of the size
               * described by this.ptrSizeof.
               */
              ptrIR: config.wasmPtrIR || 'i32',
              /**
               * True if BigInt support was enabled via (e.g.) the Emscripten
               * -sWASM_BIGINT flag, else false. When enabled, certain 64-bit
               * sqlite3 APIs are enabled which are not otherwise enabled due to
               * JS/WASM int64 impedence mismatches.
               */
               ...
               * initalizes the heap and imports it into wasm, as opposed to the
               * other way around. In this case, the memory is not available via
               * this.exports.memory.
               */
              memory:
                config.memory ||
           ...
           * the values from the source TypedArray, and returns the pointer to
           * that memory. The returned pointer must eventually be passed to
...

          {
            ...
            wasm.realloc = function f(m, n) {
              ...
              return n
                ? m2 ||
                    WasmAllocError.toss('Failed to reallocate', n, ' bytes.')
                : 0;
            };
          ...
          /**
           * Sqlite3.wasm.pstack (pseudo-stack) holds a special-case stack-style
           * allocator intended only for use with _small_ data of not more than
           * (in total) a few kb in size, managed as if it were stack-based.
           *
          ...
          wasm.pstack = Object.assign(Object.create(null), {
             ...
             * On success, it zeroes out a block of memory of the given size,
             * adjusts the pstack pointer, and returns a pointer to the memory.
             * On error, throws a WasmAllocError. The memory must eventually be
             * released using restore().
             *
             ...
             * regardless of whether the function throws. Returns the result of
             * the call or propagates an exception on error.
             *
             *        Added in 3.44.
             */
            call: function (f) {
          ...
          Object.defineProperties(wasm.pstack, {
             ...
             * position pointer. This value is intended _only_ to be saved for
             * passing to restore(). Writing to this memory, without first
             * reserving it via wasm.pstack.alloc() and friends, leads to
             * undefined results.
             */
            pointer: {
              configurable: false,
            ...
            /**
             * Sqlite3.wasm.pstack.quota to the total number of bytes available
             * in the pstack, including any space which is currently allocated.
           ...
           * directory, its path is returned by this function. If it does not
           * then it returns "" (noting that "" is a falsy value).
           *
           ...
           * (defaulting to "main"), returns a truthy value (see below) if that
           * db uses that VFS, else returns false. If pDb is falsy then the 3rd
           * argument is ignored and this function returns a truthy value if the
           * default VFS name matches that of the 2nd argument. Results are
           * undefined if pDb is truthy but refers to an invalid pointer. The
           * 3rd argument specifies the database name of the given database
           * connection to check, defaulting to the main db.
           *
          ...
          capi.sqlite3_js_db_export = function (pDb, schema = 0) {
            ...
            try {
              ...
              const ppOut = pSize + 8;
              /**
               * Maintenance reminder, since this cost a full hour of grief and
               * confusion: if the order of pSize/ppOut are reversed in that
               * memory block, fetching the value of pSize after the export
               * reads a garbage size because it's not on an 8-byte memory
               * boundary!
               */
          ...
          /**
           * Given a `sqlite3*` and a database name (JS string or WASM C-string
           * pointer, which may be 0), returns a pointer to the sqlite3_vfs
           * responsible for it. If the given db name is null/0, or not
           * provided, then "main" is assumed.
           */
           ...
           * function returns 0. As a special case, if n is falsy it does _not_
           * throw if that function returns 0. That special case is intended for
           * use with xFinal() implementations.
           */
          ...
          /**
           * If the current environment supports the POSIX file APIs, this
           * routine creates (or overwrites) the given file using those APIs.
           * This is primarily intended for use in Emscripten-based builds where
           * the POSIX APIs are transparently proxied by an in-memory virtual
           * filesystem. It may behave diffrently in other environments.
           *
          ...
          /**
           * Deprecation warning: this function does not work properly in debug
           * builds of sqlite3 because its out-of-scope use of the sqlite3_vfs
           * API triggers assertions in the core library. That was unfortunately
           * not discovered until 2023-08-11. This function is now deprecated
          ...
          /**
           * Converts SQL input from a variety of convenient formats to plain
           * strings.
           *
          ...
          if (util.isUIThread()) {
             ...
             * argument should be one of ('local','session',"").
             */
             ...
             * 'local', or "". In the first two cases, only sessionStorage resp.
             * localStorage is cleared. If it's an empty string (the default)
             * then both are cleared. Only storage keys which match the pattern
             * used by kvvfs are cleared: any other client-side data are
             * retained.
             *
             ...
             * 'local', ""). In the first two cases, only sessionStorage resp.
             * localStorage is counted. If it's an empty string (the default)
             * then both are counted. Only storage keys which match the pattern
             * used by kvvfs are counted. The returned value is the "length"
             * value of every matching key and value, noting that JavaScript
             * stores each character in 2 bytes.
             *
          ...
          /**
           * Given a (sqlite3_value*), this function attempts to convert it to
           * an equivalent JS value with as much fidelity as feasible and return
           * it.
           *
          ...
          /**
           * Requires a C-style array of `sqlite3_value*` objects and the number
           * of entries in that array. Returns a JS array containing the results
           * of passing each C array entry to sqlite3_value_to_js(). The 3rd
           * argument to this function is passed on as the 2nd argument to that
           * one.
           */
          capi.sqlite3_values_to_js = function (
            ...
            for (i = 0; i < argc; ++i) {
              /**
               * Curiously: despite ostensibly requiring 8-byte alignment, the
               * pArgv array is parcelled into chunks of 4 bytes (1 pointer
               * each). The values those point to have 8-byte alignment but the
               * individual argv entries do not.
               */
              tgt.push(
           ...
           * second arugment is coerced to a string to create the error
           * message.
           *
         ...
         the exception's type name in the result. */ capi.sqlite3_result_error(
           ...
           * sqlite3_value_to_js(). The 3rd argument of this function is ignored
           * by this function except to pass it on as the second argument of
           * sqlite3_value_to_js(). If the sqlite3_column_value() returns NULL
           * (e.g. because the column index is out of range), this function
           * returns `undefined`, regardless of the 3rd argument. If the 3rd
           * argument is falsy and conversion fails, `undefined` will be
           * returned.
           *
           ...
           * sqlite3_value at the given index and returns the result of passing
           * it to sqlite3_value_to_js(). Throws on error.
           */
          ...
          const sqlite3 = {
            ...
            config,
            /**
             * Holds the version info of the sqlite3 source tree from which the
             * generated sqlite3-api.js gets built. Note that its version may
             * well differ from that reported by sqlite3_libversion(), but that
            ...
            /**
             * This function is not part of the public interface, but a piece of
             * internal bootstrapping infrastructure.
             *
             ...
             * state which must be collected before sqlite3ApiBootstrap() can be
             * declared. It is not necessarily available to any
       ...
       up and disappear into the abyss of Promises and Workers. */
         ...
         * used by the various pieces of the sqlite3 API's amalgamation process.
         * It must not be modified by client code except when plugging such code
         ...
         * time, each such function will be called (in their appended order) and
         * passed the sqlite3 namespace object, into which they can install
        ...
        /**
         * GlobalThis.sqlite3ApiBootstrap.initializersAsync is an internal
         * detail used by the sqlite3 API's amalgamation process. It must not be
         ...
         * the asyncPostInit() process (at an indeterminate point because all of
         * them are run asynchronously in parallel).
         *
         * This list is not processed until the client calls
         * sqlite3.asyncPostInit(). This means, for example, that intializers
         * added to globalThis.sqlite3ApiBootstrap.initializers may push entries
         * to this list.
         */
         ...
         *
         * Maintenance reminder: If you're reading this in a tree other than one
         * of those listed above, note that this copy may be replaced with
         ...
         */
        /**
         * This function is intended to simplify porting around various bits of
         * WASM-related utility code from project to project.
         *
         * The primary goal of this code is to replace, where possible,
         * Emscripten-generated glue code with equivalent utility code which can
         * be used in arbitrary WASM environments built with toolchains other
         * than Emscripten. As of this writing, this code is capable of acting
         * as a replacement for Emscripten's generated glue code _except_ that
         * the latter installs handlers for Emscripten-provided APIs such as its
         * "FS" (virtual filesystem) API. Loading of such things still requires
         * using Emscripten's glue, but the post-load utility APIs provided by
         * this code are still usable as replacements for their
         * sub-optimally-documented Emscripten counterparts.
         *
         * Intended usage:
         *
         ...
         * make it available to 3rd-party scripts and "should" be deleted after
         * calling it. That symbols is _not_ used within the library.
         *
         * Forewarning: this API explicitly targets only browser environments.
         * If a given non-browser environment has the capabilities needed for a
         * given feature (e.g. TextEncoder), great, but it does not go out of
         * its way to account for them and does not provide compatibility
         * crutches for them.
         *
         ...
         *   environment-specific. Most of the following features only work if
         *   allocation is available.
         * - WASM-exported "indirect function table" access and manipulation. e.g.
         *   creating new WASM-side functions using JS functions, analog to
         *   Emscripten's addFunction() and uninstallFunction() but slightly
         *   different.
         ...
         *   closed. This is fundamentally similar to Emscripten's stackAlloc()
         *   and friends but uses the heap instead of the stack because access
         *   to the stack requires C code.
         * - Create JS wrappers for WASM functions, analog to Emscripten's ccall()
         *   and cwrap() functions, except that the automatic conversions for
         *   function arguments and return values can be easily customized by
         *   the client by assigning custom function signature type names to
         *   conversion functions. Essentially, it's ccall() and cwrap() on
         *   steroids.
         *
         * How to install...
         *
         ...
         *
         * - `exports` must be a property of the target object OR a property of
         ...
         *   following symbols:
         *
         *   - `memory`: a WebAssembly.Memory object representing the WASM memory.
         *       _Alternately_, the `memory` property can be set as
         ...
         *   - `__indirect_function_table`: the WebAssembly.Table object which holds
         *       WASM-exported functions. This API does not strictly require
         *       that the table be able to grow but it will throw if its
         *       `installFunction()` is called and the table cannot grow.
         *
         * In order to simplify downstream usage, if `target.exports` is not set
         * when this is called then a property access interceptor (read-only,
         * configurable, enumerable) gets installed as `exports` which resolves
         * to `target.instance.exports`, noting that the latter property need
         * not exist until the first time `target.exports` is accessed.
         *
         * Some APIs _optionally_ make use of the `bigIntEnabled` property of
         * the target object. It "should" be set to true if the WASM environment
         * is compiled with BigInt support, else it must be false. If it is
         * false, certain BigInt-related features will trigger an exception if
         * invoked. This property, if not set when this is called, will get a
         * default value of true only if the BigInt64Array constructor is
         * available, else it will default to false. Note that having the BigInt
         * type is not sufficient for full int64 integration with WASM: the
         * target WASM file must also have been built with that support. In
         * Emscripten that's done using the `-sWASM_BIGINT` flag.
         *
         * Some optional APIs require that the target have the following
         * methods:
         *
         * - 'alloc()`must behave like C's`malloc()`, allocating N bytes of memory
         *   and returning its pointer. In Emscripten this is conventionally
         *   made available via `Module['_malloc']`. This API requires that the
         *   alloc routine throw on allocation error, as opposed to returning
         *   null or 0.
         * - 'dealloc()`must behave like C's`free()`, accepting either a pointer
         *   returned from its allocation counterpart or the values null/0 (for
         *   which it must be a no-op). allocating N bytes of memory and
         *   returning its pointer. In Emscripten this is conventionally made
         *   available via `Module['_free']`.
         *
        ...
        globalThis.WhWasmUtilInstaller = function (target) {
           ...
           */
          /**
           * If(target.exports){ //Maybe auto-install alloc()/dealloc()...
           * if(!target.alloc && target.exports.malloc){ target.alloc =
           * function(n){ const m = this(n); return m || toss("Allocation
           * of",n,"byte(s) failed."); }.bind(target.exports.malloc); }
           *
          ...
          /**
           * Previously-recorded size of cache.memory.buffer, noted so that we
           * can recreate the view objects if the heap grows.
           */
          cache.heapSize = 0;
          /**
           * WebAssembly.Memory object extracted from target.memory or
           * target.exports.memory the first time heapWrappers() is called.
           */
          cache.memory = null;
          /**
           ...
           * 'float', 'i64', 'f64', 'double', '_'), or any string value ending
           * in '_', returns the sizeof for that value (target.ptrSizeof in the
           * latter case). For any other value, it returns the undefined value.
           */
          target.sizeofIR = (n) => {
            switch (n) {
              case 'i8':
          ...
          /**
           * If (cache.heapSize !== cache.memory.buffer.byteLength), i.e. if the
           * heap has grown since the last call, updates cache.HEAPxyz. Returns
           * the cache object.
           */
          ...
          target.functionTable = function () {
             ...
             * From Emscripten release notes from 2020-09-10: - Use
             * `__indirect_function_table` as the import name for the table,
             * which is what LLVM does.
             */
          };

          /**
           * Given a function pointer, returns the WASM function table entry if
           * found, else returns a falsy value: undefined if fptr is out of
           ...
           * returns the JS binding of that WASM function. The signature string
           * must be the Jaccwabyt-format or Emscripten addFunction()-format
           * function signature string. In short: in may have one of the
           * following formats:
           *
          ...
          target.jsFuncToWasm = function f(func, sig) {
            ...
            if (!f._) {
              ...
              f._ = {
                 ...
                 * tgt, as a little-endian value, using the given method ('push'
                 * or 'unshift').
                 */
                ...
                /**
                 * Returns the IR value for the given letter or throws if the
                 * letter is invalid.
                 */
                letterType: (x) =>
                 ...
                 */
                /**
                 * // only valid for use with the WebAssembly.Function ctor,
                 * which // is not yet documented on MDN. sigToWasm:
                 * function(sig){ const rc = {parameters:[], results: []};
                 * if('v'!==sig[0]) rc.results.push(f.sigTypes(sig[0]));
                 * for(const x of f._.sigParams(sig)){
                 * rc.parameters.push(f._.typeCodes(x)); } return rc;
                 * },***********
                 */
                /**
                 * Pushes the WASM data type code for the given signature letter
                 * to the given target array. Throws if letter is invalid.
                 */
          ...
          /**
           * Documented as target.installFunction() except for the 3rd argument:
           * if truthy, the newly-created function pointer is stashed in the
           * current scoped-alloc scope and will be cleaned up at the matching
           * scopedAllocPop(), else it is not stashed there.
           */
           ...
           * available slot of this.functionTable(), and returns the function's
           * index in that table (which acts as a pointer to that function). The
           * returned pointer can be passed to uninstallFunction() to uninstall
           * it and free up the table slot for reuse.
           *
           ...
           * scopedAllocPush() is active and uninstalls the given function when
           * that alloc scope is popped via scopedAllocPop(). This is used for
           * implementing JS/WASM function bindings which should only persist
           * for the life of a call into a single C-side function.
           */
           ...
           * installFunction() has been called and results are undefined if ptr
           * was not returned by that function. The returned function may be
           ...
           * (i8, i16, i32, i64, float (or f32), double (or f64)), this fetches
           * the numeric value from that address and returns it as a number or,
           * for the case of type='i64', a BigInt (noting that that type
           * triggers an exception if this.bigIntEnabled is falsy). Throws if
           ...
           * WASM heap address, using the type to define how many bytes are
           * written. Throws if given an invalid type. See peek() for details
           * about the type argument. If the 3rd argument ends with `*` then it
           * is treated as a pointer type and this function behaves as if the
           * 3rd argument were `i32`.
           *
          ...
          /**
           * Convenience form of peek() intended for fetching pointer-to-pointer
           * values. If passed a single non-array argument it returns the value
           * of that one pointer address. If passed multiple arguments, or a
           * single array of arguments, it returns an array of their values.
           */
          target.peekPtr = (...ptr) =>
            target.peek(1 === ptr.length ? ptr[0] : ptr, ptrIR);

          /**
           * A variant of poke() intended for setting pointer-to-pointer values.
           * Its differences from poke() are that (1) it defaults to a value of
           * 0 and (2) it always writes to the pointer-sized heap view.
           */
          target.pokePtr = (ptr, value = 0) => target.poke(ptr, value, ptrIR);

          /**
           * Convenience form of peek() intended for fetching i8 values. If
           * passed a single non-array argument it returns the value of that one
           * pointer address. If passed multiple arguments, or a single array of
           * arguments, it returns an array of their values.
           */
          target.peek8 = (...ptr) =>
            target.peek(1 === ptr.length ? ptr[0] : ptr, 'i8');
          /**
           * Convience form of poke() intended for setting individual bytes. Its
           * difference from poke() is that it always writes to the i8-sized
           * heap view.
           */
          ...
          /**
           * I64 variant of peek8(). Will throw if this build is not configured
           * for BigInt support.
           */
          target.peek64 = (...ptr) =>
            target.peek(1 === ptr.length ? ptr[0] : ptr, 'i64');
          /**
           * I64 variant of poke8(). Will throw if this build is not configured
           * for BigInt support. Note that this returns a BigInt-type value, not
           * a Number-type value.
           */
          ...
          /**
           * Returns true if the given value appears to be legal for use as a
           * WASM pointer value. Its _range_ of values is not (cannot be)
           * validated except to ensure that it is a 32-bit integer with a value
           * of 0 or greater. Likewise, it cannot verify whether the value
           * actually refers to allocated memory in the WASM heap.
           */
          target.isPtr32 = (ptr) =>
          ...
          /**
           * Expects ptr to be a pointer into the WASM heap memory which refers
           * to a NUL-terminated C-style string encoded as UTF-8. Returns the
           * length, in bytes, of the string, as for `strlen(3)`. As a special
           * case, if !ptr or if it's not a pointer then it returns `null`.
           * Throws if ptr is out of range for target.heap8u().
           */
          ...
          /**
           * Expects ptr to be a pointer into the WASM heap memory which refers
           * to a NUL-terminated C-style string encoded as UTF-8. This function
           * counts its byte length using cstrlen() then returns a JS-format
           * string representing its contents. As a special case, if ptr is
           * falsy or not a pointer, `null` is returned.
           */
          ...
          /**
           * Encodes the given JS string as UTF8 into the given TypedArray tgt,
           * starting at the given offset and writing, at most, maxBytes bytes
           * (including the NUL terminator if addNul is true, else no NUL is
           * added). If it writes any bytes at all and addNul is true, it always
           * NUL-terminates the output, even if doing so means that the NUL byte
           * is all that it writes.
           *
           ...
           * encoded as UTF-8. If addNul is true, the returned array will have a
           ...
           * value returned from this function is passed to scopedAllocPop().
           *
           ...
           * of the given opaque state object, which must be a value returned by
           * scopedAllocPush(). See that function for an example of how to use
           * this function.
           *
           ...
           * Ownership of the memory is given to scopedAllocPop(), which will
           * clean it up when it is called. The memory _must not_ be passed to
           * this.dealloc(). Throws if this API object is missing the required
           * `alloc()` or `dealloc()` functions or no scoped alloc is active.
           *
           ...
           * property and a forEach() method. A block of memory (list.length+1)
           ...
           * (""+value) of each element, with the exception that the final entry
           * is a NULL pointer. Returns a pointer to the start of the list,
           * suitable for passing as the 2nd argument to a C-style main()
           * function.
           *
           ...
           * returns a JS array of strings and/or nulls: any entry in the pArgv
           * array which is NULL results in a null entry in the result array. If
           * argc is 0 then an empty array is returned.
           *
           ...
           * automatically when func() returns. If func throws or propagates an
           * exception, the scope is still popped, otherwise it returns the
          ...
          /**
           * Identical to allocPtr() except that it allocates using
           * scopedAlloc() instead of alloc().
           */
          target.scopedAllocPtr = (howMany = 1, safePtrSize = true) =>
            __allocPtr(howMany, safePtrSize, 'scopedAlloc');

          /**
           * If target.exports[name] exists, it is returned, else an exception
           * is thrown.
           */
          ...
          /**
           * Looks up a WASM-exported function named fname from target.exports.
           * If found, it is called, passed all remaining arguments, and its
           * return value is returned to xCall's caller. If not found, an
           * exception is thrown. This function does no conversion of argument
           * or return types, but see xWrap() and xCallWrapped() for variants
           * which do.
           *
       ...
       add pointer-style variants of them. */
          ...
          /**
           * In order for args of type string to work in various contexts in the
           * sqlite3 API, we need to pass them on as, variably, a C-string or a
           * pointer value. Thus for ARGs of type 'string' and '*'/'pointer' we
           * behave differently depending on whether the argument is a string or
           * not:
           *
          ...
          const AbstractArgAdapter = class {
            ...
            /**
             * Gets called via xWrap() to "convert" v to whatever type this
           ...
           * for the duration of a single call into the C layer, or
           * semi-contextual, where it may keep track of a single binding for a
           * given context and uninstall the binding if it's replaced.
           *
          ...
          xArg.FuncPtrAdapter = class FuncPtrAdapter extends (
             ...
             * to work around an emcc toolchain build problem: one of the tools
       ...
       may be anything suitable for use in a Map. */
             ...
             * pointer. If v is one of (a pointer, null, undefined) then (v||0)
             * is returned and any earlier function installed by this mapping
             * _might_, depending on how it's bound, be uninstalled. If v is not
             * one of those types, it must be a Function, for which it creates
             * (if needed) a WASM function binding and returns the WASM pointer
             * to that binding. If this instance is not in 'transient' mode, it
             * will remember the binding for at least the next call, to avoid
             * recreating the function binding unnecessarily.
             *
          ...
          /**
           * If true, the constructor emits a warning. The intent is that this
           * be set to true after bootstrapping of the higher-level client
           * library is complete, to warn downstream clients that they shouldn't
           * be relying on this implemenation detail which does not have a
           * stable interface.
           */
           ...
           * then converts the wrapped function's result to another form for the
           * wrapper.
           *
        ...
        */
          ...
          /**
           * Gets, sets, or removes a result value adapter for use with xWrap().
           * If passed only 1 argument, the adapter function for the given type
           * name is returned. If the second argument is explicit falsy (as
           * opposed to defaulted), the adapter named by the first argument is
           * removed. If the 2nd argument is not falsy, it must be a function
           * which takes one value and returns a value appropriate for the given
           * type name. The adapter may throw if its argument is not of a type
           * it can work with. This function throws for invalid arguments.
           *
           *      Example:
           *
           *      ```
          ...
          /**
           * Functions identically to xWrap.resultAdapter() but applies to call
           * argument conversions instead of result value conversions.
           *
           ...
           * conversions as for xWrap(). The first, second, and third arguments
           * are as documented for xWrap(), except that the 3rd argument may be
           * either a falsy value or empty array to represent nullary functions.
           * The 4th+ arguments are arguments for the call, with the special
           * case that if the 4th argument is an array, it is used as the
           * arguments for the call. Returns the converted result of the call.
           *
           ...
           * testing. It should not be used in application-level code, only in
           * test code.
           *
        ...
        /**
         * Yawl (Yet Another Wasm Loader) provides very basic wasm loader. It
         * requires a config object:
         *
         * - `uri`: required URI of the WASM file to load.
         * - `onload(loadResult,config)`: optional callback. The first argument is
         *   the result object from WebAssembly.instantiate[Streaming](). The
         *   2nd is the config object passed to this function. Described in more
         *   detail below.
         * - `imports`: optional imports object for
         *   WebAssembly.instantiate[Streaming](). The default is an empty set
         *   of imports. If the module requires any imports, this object must
         *   include them.
         ...
         *   - If `wasmUtilTarget.alloc` is not set and `instance.exports.malloc`
         *       is, it installs `wasmUtilTarget.alloc()` and
         *       `wasmUtilTarget.dealloc()` wrappers for the exports `malloc`
         *       and `free` functions.
         *
...
*/
          ...
          /**
           * Throws a new Error, the message of which is the concatenation all
           * args with a space between each.
           */
          const toss = (...args) => {
            throw new Error(args.join(' '));
          };
          ...
          /**
           * Implementing function bindings revealed significant shortcomings in
           * Emscripten's addFunction()/removeFunction() interfaces:
           *
           *      https://github.com/emscripten-core/emscripten/issues/17323
           *
          ...
          /** True if SIG s looks like a function signature, else false. */
          ...
          /**
           * Returns the WASM IR form of the Emscripten-conventional letter at
           * SIG s[0]. Throws for an unknown SIG.
           */
          ...
          /**
           * Returns the name of a DataView getter method corresponding to the
           * given SIG.
           */
          ...
          /**
           * Returns the name of a DataView setter method corresponding to the
           * given SIG.
           */
          ...
          /**
           * Returns either Number of BigInt, depending on the given SIG. This
           * constructor is used in property setters to coerce the being-set
          ...
          /**
           * In order to completely hide StructBinder-bound struct pointers from
           * JS code, we store them in a scope-local WeakMap which maps the
           * struct-bound objects to their WASM pointers. The pointers are
           * accessible via boundObject.pointer, which is gated behind an
           * accessor function, but are not exposed anywhere else in the object.
           * The main intention of that is to make it impossible for stale
           * copies to be made.
           */
          ...
          /** Frees the obj.pointer memory and clears the pointer property. */
          ...
          /**
           * Returns a skeleton for a read-only property accessor wrapping value
           * v.
           */
          const rop = (v) => {
            return {
          ...
          /**
           * Looks up a struct member in structInfo.members. Throws if found if
           * tossIfNotFound is true, else returns undefined if not found. The
           * given name may be either the name of the structInfo.members key
           * (faster) or the key as modified by the memberPrefix and
          ...
          /**
           * Uses __lookupMember() to find the given obj.structInfo key. Returns
           * that member if it is a string, else returns false. If the member is
           * not found, throws if tossIfNotFound is true, else returns false.
           */
           ...
           * It optimistically assumes that any signature of (i,p,s) is C-string
           * compatible.
           */
          ...
          /**
           * Looks up the given member in obj.structInfo. If it has a signature
           * of 's' then it is assumed to be a C-style UTF-8 string and a
           * decoded copy of the string at its address is returned. If the
           * signature is of any other type, it throws. If an s-type member's
           * address is 0, `null` is returned.
           */
          ...
          /**
           * Adds value v to obj.ondispose, creating ondispose, or converting it
           ...
           * string and returns its address relative to heap(). If allocation
           * returns 0 this function throws. Ownership of the memory is
           * transfered to the caller, who must eventually pass it to the
           * configured dealloc() function.
           */
       ...
       dealloc that value and clear that ondispose entry */
          ...
          /**
           * Pass this a StructBinder-generated prototype, and the struct member
           * description object. It will define property accessors for
           * proto[memberKey] which read from/write to memory in this.pointer.
           * It modifies descr to make certain downstream operations much
           * simpler.
           */
    ...
    */
          ...
          /** The main factory function which will be returned to the caller. */
...
*/
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
          ...
          if (0) {
            /**
             * Please keep this block around as a maintenance reminder that we
             * cannot rely on this type of check.
             *
             ...
             */
            /**
             * Find a mapping for SQLITE_WASM_DEALLOC, which the API guarantees
             * is a WASM pointer to the same underlying function as
             * wasm.dealloc() (noting that wasm.dealloc() is permitted to be a
             * JS wrapper around the WASM function). There is unfortunately no
             * O(1) algorithm for finding this pointer: we have to walk the WASM
             * indirect function table to find it. However, experience indicates
             * that that particular function is always very close to the front
             * of the table (it's been entry #3 in all relevant tests).
             */
          ...
          /**
           * Signatures for the WASM-exported C-side functions. Each entry is an
    ...
    */
       ...
       optional features into account. */
           ...
           * (That said: we never actually build without BigInt support, and
           * such builds are untested.)
           *
      ...
      proxy.] */
          ...
          if (wasm.bigIntEnabled && !!wasm.exports.sqlite3changegroup_add) {
             ...
             * from a C string to a JS string before passing the arguments on to
             * the client-provided JS callback.
             */
            const __ipsProxy = {
              signature: 'i(ps)',
              callProxy: (callback) => {
           ...
           * sqlite3.util. Some of them get exposed to clients via variants in
           * sqlite3_js_...().
           *
       ...
       'string:flexible'-type arguments */
             ...
             * wasm.bindingSignatures and (B) provide automatic conversion from
       ...
       implicitly making it part of the public interface. */
          ...
          /**
           * Internal helper to assist in validating call argument counts in the
           * hand-written sqlite3_xyz() wrappers. We do this only for
          ...
          /**
           * __dbCleanupMap is infrastructure for recording registration of UDFs
           * and collations so that sqlite3_close_v2() can clean up any
          ...
          /**
           * Intended to be called _only_ from sqlite3_close_v2(), passed its
           * non-0 db argument.
           *
          ...
          __dbCleanupMap.cleanup = function (pDb) {
             ...
             * bindings. The FuncPtrAdapter which sits between us and the C API
             * will also treat that as an opportunity to
      ...
      and sqlite3_create_window_function(). */
            /**
             * FuncPtrAdapter for contextKey() for sqlite3_create_function() and
             * friends.
             */
             ...
             * callbacks, structured in a form usable by
            ...
            /**
             * A _deprecated_ alias for capi.sqlite3_result_js() which predates
             * the addition of that function in the public API.
             */
      ...
      sqlite3_prepare_v3() */
             ...
             * ostensible length, this function returns [V,N], where V is either
             * v or a transformed copy of v and N is either n, -1, or the byte
            ...
            const __prepare = {
              /**
               * This binding expects a JS string as its 2nd argument and null
               * as its final argument. In order to compile multiple statements
               * from a single string, the "full" impl (see below) must be
               * used.
               */
               ...
               * SQL string, instead of being converted to a string. This
               * variant is necessary for cases where we require a non-NULL
               * value for the final argument (exec()'ing multiple statements
               * from one input string). For simpler cases, where only the first
               * statement in the SQL string is required, the wrapper named
               * sqlite3_prepare_v2() is sufficient and easier to use because it
               * doesn't require dealing with pointers.
               */
     ...
     object during the final steps of the API cleanup. */ /**
           ...
           * function in the given StructBinder.StructType target object.
           *
         ...
         StructBinder-created types. */
            ...
            const fProxy =
              applyArgcCheck && !wasm.isPtr(func)
                ? /**
                   * This middle-man proxy is only for use during development, to confirm that we
                   * always pass the proper number of arguments. We know that the C-level code
                   * will always use the correct argument count.
                   */
          ...
          /**
           * Equivalent to calling installMethod(this,...arguments) with a first
           * argument of this object. If called with 1 or 2 arguments and the
           * first is an object, it's instead equivalent to calling
          ...
          /**
           * Equivalent to calling installMethods() with a first argument of
           * this object.
           */
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
          sqlite3.version = {
            ...
            sourceId:
              '2024-03-07 12:34:26 720ce06d93a9e4cc25c34c873c82165d8801f208c22701e51538f3210de8alt1',
...
*/
     ...
     and the sqlite3 API is in a worker. */
          ...
          /**
           * In order to keep clients from manipulating, perhaps inadvertently,
           * the underlying pointer values of DB and Stmt instances, we'll gate
           * access to them via the `pointer` property accessor and store their
           * real values in this map. Keys = DB/Stmt objects, values = pointer
           * values. This also unifies how those are accessed, for potential use
           * downstream via custom wasm.xWrap() function signatures which know
           * how to extract it.
           */
          ...
          /**
           * If object opts has _its own_ property named p then that property's
           * value is returned, else dflt is returned.
           */
          ...
          /**
           * Sqlite3_trace_v2() callback which gets installed by the DB ctor if
           * its open-flags contain "t".
           */
          ...
          /**
           * A map of sqlite3_vfs pointers to SQL code or a callback function to
           * run when the DB constructor opens a database with the given VFS. In
           * the latter case, the call signature is
           ...
           * being-construct DB object as its "this". See the DB constructor for
           * the argument docs. This is split into a separate function in order
           * to enable simple creation of special-case DB constructors, e.g.
           * JsStorageDb and OpfsDb.
           *
          ...
          const dbCtorHelper = function ctor(...args) {
            if (!ctor._name2vfs) {
              /**
               * Map special filenames which we handle here (instead of in C) to
           ...
           * is called with (theOo1DbObject,sqlite3Namespace) at the end of the
           * DB() constructor. The function must throw on error, in which case
           * the db is closed and the exception is propagated. This function is
          ...
          /**
           * The DB class provides a high-level OO wrapper around an sqlite3 db
           * handle.
           *
          ...
          /**
           * Internal-use enum for mapping JS types to DB-bindable types. These
           * do not (and need not) line up with the SQLITE_type values. All
           * values in this enum must be truthy and distinct but they need not
           * be numbers.
           */
          const BindTypes = {
            null: 1,
          ...
          /**
           * This class wraps sqlite3_stmt. Calling this constructor directly
           * will trigger an exception. Use DB.prepare() to create new
           * instances.
           *
          ...
          /**
           * Throws if ndx is not an integer or if it is out of range for
           * stmt.columnCount, else returns stmt.
           *
           ...
           * the argument processing/validation, throws on error, and returns a
          ...
          /** Internal impl of the DB.selectArrays() and selectObjects() methods. */
          ...
          /**
           * Expects to be given a DB instance or an `sqlite3*` pointer (may be
           * null) and an sqlite3 API result code. If the result code is not
           * falsy, this function throws an SQLite3Error with an error message
           * from sqlite3_errmsg(), using db (or, if db is-a DB, db.pointer) as
           * the db handle, or sqlite3_errstr() if db is falsy. Note that if
           * it's passed a non-error code like SQLITE_ROW or SQLITE_DONE, it
           * will still throw but the error string might be "Not an error." The
           * various non-0 non-error codes need to be checked for in client code
          ...
          DB.prototype = {
             ...
             * connection. This is a no-op if the db has already been closed.
             * After calling close(), `this.pointer` will resolve to
             * `undefined`, so that can be used to check whether the db instance
             * is still opened.
             *
            ...
            /**
             * Returns the number of changes, as per sqlite3_changes() (if the
             * first argument is false) or sqlite3_total_changes() (if it's
             * true). If the 2nd argument is true, it uses sqlite3_changes64()
             * or sqlite3_total_changes64(), which will trigger an exception if
             * this build does not have BigInt support enabled.
             */
             ...
             * defaulting to "main". The argument may be either a JS string or a
             * pointer to a WASM-allocated C-string.
             */
            ...
            /**
             * Returns the name of the sqlite3_vfs used by the given database of
             * this connection (defaulting to 'main'). The argument may be
            ...
            /**
             * Compiles the given SQL and returns a prepared Stmt. This is the
             * only way to create new Stmt objects. Throws on error.
             *
             ...
             * contain the SQL to execute. By default it returns this object but
             * that can be changed via the `returnValue` option as described
             * below. Throws on error.
             *
           ...
           will point to somewhere in pSql. */
                ...
                const pzTail =
                  ppStmt +
                  wasm.ptrSizeof; /* final arg to sqlite3_prepare_v2() */
        ...
        throw e;
            ...
            /**
             * Creates a new UDF (User-Defined Function) which is accessible via
             * SQL code. This function may be called in any of the following
             * forms:
             *
             *        - (name, function)
             ...
             * value of the first result column. If it has no results, undefined
             * is returned.
             *
            ...
            /**
             * Runs the given query and returns an array of the values from the
             * first result column of each row of the result set. The 2nd
             ...
             * containing the values of the first result row. If it has no
             * results, `undefined` is returned.
             *
             ...
             * object containing the key/value pairs of the first result row. If
             * it has no results, `undefined` is returned.
             *
            ...
            /**
             * Runs the given SQL and returns an array of all results, with each
             * row represented as an array, as per the 'array' `rowMode` option
             * to `exec()`. An empty result set resolves to an empty array. The
             * second argument, if any, is treated as the 'bind' option to a
             * call to exec().
             */
            ...
            /**
             * Works identically to selectArrays() except that each value in the
             * returned array is an object, as per the 'object' `rowMode` option
             * to `exec()`.
             */
             ...
             * handles prepared via this.prepare() are counted, and not handles
             * prepared using capi.sqlite3_prepare_v3() (or equivalent).
             */
             ...
             * only argument. On success, returns the result of the callback.
             * Throws on error.
             *
            ...
            /**
             * This works similarly to transaction() but uses sqlite3's
             * SAVEPOINT feature. This function starts a savepoint (with an
             * unspecified name) and calls the given callback function, passing
             * it this db object. If the callback returns, the savepoint is
             * released (committed). If the callback throws, the savepoint is
             * rolled back. If it does not throw, it returns the result of the
             * callback.
             */
            ...
            /**
             * A convenience form of DB.checkRc(this,resultCode). If it does not
             * throw, it returns this object.
             */
          ...
          /** Throws if the given Stmt has been finalized, else stmt is returned. */
          ...
          /**
           * Returns an opaque truthy value from the BindTypes enum if v's type
           * is a valid bindable type, else returns a falsy value. As a special
           * case, a value of undefined is treated as a bind type of null.
           */
          ...
          /**
           * If isSupportedBindType(v) returns a truthy value, this function
           * returns that value, else it throws.
           */
           ...
           * complaining that the 2nd argument (an operation name, e.g.
           * "bind()") is not legal while the statement is "locked". Locking
           * happens before an exec()-like callback is passed a statement, to
           * ensure that the callback does not mutate or finalize the statement.
           * If it does not throw, it returns stmt.
           */
          ...
          /**
           * Binds a single bound parameter value on the given stmt at the given
           * index (numeric or named) using the given bindType (see the
           * BindTypes enum) and value. Throws on error. Returns stmt on
           * success.
           */
          ...
          Stmt.prototype = {
             ...
             * already been finalized. Returns the result of sqlite3_finalize()
             * (0 on success, non-0 on error), or the undefined value if the
             * statement has already been finalized. Regardless of success or
             * failure, most methods in this class will throw if called after
             * this is.
             *
             ...
             * statement has been finalized or if modification of the statement
             * is currently illegal (e.g. in the per-row callback of a DB.exec()
             * call).
             */
             ...
             * beginning. Returns this object. Throws if this statement has been
             * finalized, if it may not legally be reset because it is currently
             * being used from a DB.exec() callback, or if the underlying call
             * to sqlite3_reset() returns non-0.
             *
            ...
            /**
             * Binds one or more values to its bindable parameters. It accepts 1
             * or 2 arguments:
             *
           ...
           functions. */
            ...
            /**
             * Special case of bind() which binds the given value using the BLOB
             * binding mechanism instead of the default selected one for the
             * value. The ndx may be a numbered or named bind index. The value
             * must be of type string, null/undefined (both get treated as
             * null), or a TypedArray of a type supported by the bind() API.
             * This API cannot bind numbers as blobs.
             *
            ...
            /**
             * Steps the statement one time. If the result indicates that a row
             * of data is available, a truthy value is returned. If no row of
             * data is available, a falsy value is returned. Throws on error.
             */
            step: function () {
            ...
            /**
             * Fetches the value from the given 0-based column index of the
             * current data row, throwing if index is out of range.
             *
            ...
            get: function (ndx, asType) {
              ...
              if (Array.isArray(ndx)) {
                ...
                const n = this.columnCount;
                while (i < n) {
                  ndx[capi.sqlite3_column_name(this.pointer, i)] = this.get(
                    i++,
                  );
                }
                return ndx;
            ...
            /** Equivalent to get(ndx) but coerces the result to an integer. */
            getInt: function (ndx) {
              return this.get(ndx, capi.SQLITE_INTEGER);
            },
            /** Equivalent to get(ndx) but coerces the result to a float. */
            ...
            /**
             * A convenience wrapper around get() which fetches the value as a
             * string and then, if it is not null, passes it to JSON.parse(),
             * returning that result. Throws if parsing fails. If the result is
             * null, null is returned. An empty string, on the other hand, will
            ...
            /**
             * Returns the result column name of the given index, or throws if
             * index is out of bounds or this statement has been finalized. This
             * can be used without having run step() first.
             */
             ...
             * used as the target and all names are appended to it. Returns the
             * target array. Throws if this statement cannot have result
             ...
             * name matches one, its 1-based bind index is returned. If no match
             * is found, 0 is returned. If it has no bindable parameters, the
          ...
          if (util.isUIThread()) {
             ...
             * that it throws if the given storage name is not one of 'local' or
             * 'session'.
             */
             ...
             * instance has been closed. Returns the number of database blocks
            ...
            /**
             * Returns the _approximate_ number of bytes this database takes up
             * in its storage or throws if this instance has been closed.
             */
         ...
         * thread via Worker-style messages. Because of limitations in that type
         * of communication, this API is minimalistic and only capable of
         ...
         * API requires calling initWorker1API(). If this function is called
         * from a non-worker thread then it throws an exception. It must only be
         ...
         * optionally depend on this function not returning until initialization
         * is complete, as the initialization is synchronous. In some contexts,
         * however, listening for the above message is a better fit.
         *
         * Note that the worker-based interface can be slightly quirky because
         * of its async nature. In particular, any number of messages may be
         * posted to the worker before it starts handling any of them. If, e.g.,
         * an "open" operation fails, any subsequent messages will fail. The
         * Promise-based wrapper for this API (`sqlite3-worker1-promiser.js`) is
         * more comfortable to use in that regard.
         *
         * The documentation for the input and output worker messages for this
         * API follows...
         *
         * ====================================================================
         ...
         *     }
         *
         * If the `dbId` does not refer to an opened ID, this is a no-op. If the
         * `args` object contains a truthy `unlink` value then the database will
         ...
         * most of the features of the oo1.DB.exec() method, with a few
         ...
         * evaluated.
         *
         * A function-type args.callback property cannot cross the window/Worker
         * boundary, so is not useful here. If args.callback is a string then it
         * is assumed to be a message type key, in which case a callback
         * function will be applied which posts each row result via:
         *
         ...
         * call will tie up the Worker thread, causing any recursion attempt to
         * wait until the first exec() is completed.
         *
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(function (sqlite3) {
          ...
          sqlite3.initWorker1API = function () {
            ...
            const wState = {
              ...
              /**
               * Fetch the DB for the given id. Throw if require=true and the id
               * is not valid, else return the db or undefined.
               */
            ...
            /**
             * A level of "organizational abstraction" for the Worker1 API. Each
             * method in this object must map directly to a Worker1 message type
             * key. The onmessage() dispatcher attempts to dispatch all inbound
             * messages to a method of this object, passing it the event.data
             * part of the inbound event object. All methods must return a plain
             * Object containing any result state, which the dispatcher may
             * amend. All methods must throw on error.
             */
            const wMsgHandler = {
              open: function (ev) {
        ...
        /**
         * This file installs sqlite3.vfs, a namespace of helpers for use in the
         * creation of JavaScript implementations of sqlite3_vfs.
         */
        ('use strict');
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(
          function (sqlite3) {
             ...
             * sqlite3_index_info.sqlite3_index_constraint_usage instance if
             * passed no 2nd argument or a falsy 2nd argument.
             */
            ...
            /**
             * If n is >=0 and less than this.$nOrderBy, this function returns
             * either a WASM pointer to the 0-based nth entry of this.$aOrderBy
             * (if passed a truthy 2nd argument) or an
             ...
             * The first argument must be the logical name of the manager (e.g.
             * 'xVtab' or 'xCursor'), which is only used for error reporting.
             * The second must be the capi.XYZ struct-type value, e.g.
             * capi.sqlite3_vtab or capi.sqlite3_vtab_cursor.
             *
            ...
            const StructPtrMapper = function (name, StructType) {
              ...
              return Object.assign(Object.create(null), {
                ...
                StructType,
                /**
                 * Creates a new StructType object, writes its `pointer` value
                 * to the given output pointer, and returns that object. Its
                 * intended usage depends on StructType:
                 *
                ...
                /**
                 * Returns the StructType object previously mapped to the given
                 * pointer using create(). Its intended usage depends on
                 * StructType:
                 *
                ...
                /**
                 * Identical to get() but also disconnects the mapping between
                 * the given pointer and the returned StructType object, such
                 * that future calls to this function or get() with the same
                 * pointer will return the undefined value. Its intended usage
                 * depends on StructType:
                 *
                ...
                /**
                 * Works like unget() plus it calls dispose() on the StructType
                 * object.
                 */
                dispose: (pCObj) => {
...
*/
        ...
        globalThis.sqlite3ApiBootstrap.initializers.push(
          function (sqlite3) {
             ...
             *
             * The installed VFS uses the Origin-Private FileSystem API for all
             * file storage. On error it is rejected with an exception
             * explaining the problem. Reasons for rejection include, but are
             * not limited to:
             *
             * - The counterpart Worker (see below) could not be loaded.
             * - The environment does not support OPFS. That includes when this
             *   function is called from the main window thread.
             *
             * Significant notes and limitations:
             *
             * - As of this writing, OPFS is still very much in flux and only
             *   available in bleeding-edge versions of Chrome (v102+, noting
             *   that that number will increase as the OPFS API matures).
             * - The OPFS features used here are only available in dedicated
             *   Worker threads. This file tries to detect that case, resulting
             *   in a rejected Promise if those features do not seem to be
             *   available.
             ...
             *
             * All arguments to this function are for internal/development
             * purposes only. They do not constitute a public API and may change
             * at any time.
             *
             ...
             * - SanityChecks (=false): if true, some basic sanity tests are run
             *   on the OPFS VFS API after it's initialized, before the returned
             *   Promise resolves.
             *
             * On success, the Promise resolves to the top-most sqlite3
             * namespace object and that object gets a new object installed in
             * its `opfs` property, containing several OPFS-specific utilities.
             */
            const installOpfsVfs = function callee(options) {
              ...
              const thePromise = new Promise(function (
                ...
                /**
                 * Generic utilities for working with OPFS. This will get filled
                 * out by the Promise setup and, on success, installed as
                 * sqlite3.opfs.
                 *
         ...
         https://sqlite.org/forum/forumpost/a708c98dcb3ef */
                ...
                /**
                 * Pedantic sidebar about opfsVfs.ondispose: the entries in that
                 * array are items to clean up when opfsVfs.dispose() is called,
                 * but in this environment it will never be called. The VFS
                 * instance simply hangs around until the WASM module instance
                 * is cleaned up. We "could" _hypothetically_ clean it up by
                 * "importing" an sqlite3_os_end() impl into the wasm build, but
                 * the shutdown order of the wasm engine and the JS one are
                 * undefined so there is no guaranty that the opfsVfs instance
                 * would be available in one environment or the other when
                 ...
                 * objects. After the worker's "inited" message arrives, other
                 * types of data may be added to it.
                 *
                ...
                /**
                 * AsyncIdleWaitTime is how long (ms) to wait, in the async
                 * proxy, for each Atomics.wait() when waiting on inbound VFS
                 * API calls. We need to wake up periodically to give the thread
                 * a chance to do other things. If this is too high (e.g. 500ms)
                 * then even two workers/tabs can easily run into locking
                 * errors. Some multiple of this value is also used for
                 * determining how long to wait on lock contention to free up.
                 */
                state.asyncIdleWaitTime = 150;

                /**
                 * Whether the async counterpart should log exceptions to the
                 * serialization channel. That produces a great deal of noise
                 * for seemingly innocuous things like xAccess() checks for
                 * missing files, so this option may have one of 3 values:
                 *
       ...
       xRead/xWrite() will never deal in blocks larger than that. */
                ...
                /**
                 * The size of the block in our SAB for serializing arguments
                 * and result values. Needs to be large enough to hold
                 * serialized values of any of the proxied APIs. Filenames are
                 * the largest part but are limited to opfsVfs.$mxPathname
                 * bytes. We also store exceptions there, so it needs to be long
                 * enough to hold a reasonably long exception string.
                 */
         ...
         for that, doing so might lead to undesired side effects. */
                ...
                state.opfsFlags = Object.assign(Object.create(null), {
                   ...
                   * access handle (i.e. an OPFS lock) will release that locks
                   * at the end of the call which acquires it. If false, such
                 ...
                 * and subsequent arguments must be the aruguments for the async
                 * op.
                 */
                const opRun = (op, ...args) => {
         ...
         sqlite3_vfs API. */
                  ...
                  /**
                   * Returns an array of the deserialized state stored by the
                   * most recent serialize() operation (from from this thread or
                   * the counterpart thread), or null if the serialization
                   * buffer is empty. If passed a truthy argument, the
                   * serialization buffer is cleared after deserialization.
                   */
                  ...
                  /**
                   * Serializes all arguments to the shared buffer for
                   * consumption by the counterpart thread.
                   *
               ...
               corresponding TypeIds.id from the header. */
                ...
                /**
                 * Generates a random ASCII string len characters long, intended
                 * for use as a temporary file name.
                 */
      ...
      */
                ...
                /**
                 * Map of sqlite3_file pointers to objects constructed by
                 * xOpen().
                 */
                ...
                /**
                 * Impls for the sqlite3_io_methods methods. Maintenance
                 * reminder: members are in alphabetical order to simplify
                 * finding them.
                 */
                const ioSyncWrappers = {
                  xCheckReservedLock: function (pFile, pOut) {
                    /**
                     * As of late 2022, only a single lock can be held on an
                     * OPFS file. We have no way of checking whether any _other_
                     * db connection has a lock except by trying to obtain and
                     * (on success) release a sync-handle for it, but doing so
                     * would involve an inherent race condition. For the time
                     * being, pending a better solution, we simply report
                     * whether the given pFile is open.
                     */
          ...
          mTimeEnd();*/
           ...
           lock the file. */
                  ...
                  xRead: function (pFile, pDest, n, offset64) {
                    ...
                    try {
                      ...
                      if (0 === rc || capi.SQLITE_IOERR_SHORT_READ === rc) {
                        /**
                         * Results get written to the SharedArrayBuffer
                         * f.sabView. Because the heap is _not_ a
                         * SharedArrayBuffer, we have to copy the results.
                         * TypedArray.set() seems to be the fastest way to copy
                         * this.
                         */
                ...
                /**
                 * Impls for the sqlite3_vfs methods. Maintenance reminder:
         ...
         one. */
                 ...
                 * arg is true, the result is returned as an array of path
                 * elements, else an absolute path string is returned.
                 */
                 ...
                 * array of [handleOfContainingDir, filename]. If the 2nd
                 * argument is truthy then each directory element leading to the
                 * file is created along the way. Throws if any creation or
                 * resolution fails.
                 */
                ...
                /**
                 * Creates the given directory name, recursively, in the OPFS
                 * filesystem. Returns true if it succeeds or the directory
                 ...
                 * temporary file name. Its argument is the length of the
                 * string, defaulting to 16.
                 */
                opfsUtil.randomFilename = randomFilename;

                /**
                 * Returns a promise which resolves to an object which
                 * represents all files and directories in the OPFS tree. The
                 * top-most object has two properties: `dirs` is an array of
                 * directory entries (described below) and `files` is a list of
                 * file names for all files in that directory.
                 *
                 ...
                 * locked), but the precise conditions under which the
                 * underlying APIs will throw are not documented (so we cannot
                 ...
                 * has no notion of "current directory", the given name must be
                 * an absolute path. If the 2nd argument is truthy, deletion is
                 ...
                 * one. The argument may be either a callback function or an
                ...
                const importDbChunked = async function (filename, callback) {
                  ...
                  try {
                    ...
                    sah.write(
                      new Uint8Array([1, 1]),
                      { at: 18 } /*force db out of WAL mode*/,
                    );
                    return nWrote;
                  } catch (e) {
                    await sah.close();
...
*/
     ...
     they will be cleaned up during VFS init. */
            ...
            /**
             * Returns short a string of random alphanumeric characters suitable
             * for use as a random filename.
             */
       ...
       currently-opened client-specified filenames. */
              ...
              /**
               * Adds n files to the pool's capacity. This change is persistent
               * across settings. Returns a Promise which resolves to the new
               * capacity.
               */
              async addCapacity(n) {
                for (let i = 0; i < n; ++i) {
               ...
               * the number of slots really removed.
               */
               ...
               * after this is acquireAccessHandles().
               */
              ...
              /**
               * Given an SAH, returns the client-specified name of that file by
               * extracting it from the SAH's header.
               *
              ...
              /**
               * Returns the pathname part of the given argument, which may be
               * any of:
               *
               *        - a URL object
              ...
              /**
               * Removes the association of the given client-specified file name
               * (JS string) from the pool. Returns true if a mapping is found,
               * else false.
               */
              deletePath(path) {
              ...
              /**
               * Pops this object's Error object and returns it (a falsy value
               * if no error is set).
               */
              popErr() {
                const rc = this.$error;
              ...
              /**
               * Returns the next available SAH without removing it from the
               * set.
               */
              nextAvailableSAH() {
              ...
              /** Given an (sqlite3_file*), returns the mapped xOpen file object. */
              getOFileForS3File(pFile) {
                return this.#mapS3FileToOFile_.get(pFile);
              }
              /**
               * Maps or unmaps (if file is falsy) the given (sqlite3_file*) to
               * an xOpen file object and to this pool object.
               */
              ...
              /**
               * Returns the SAH associated with the given client-defined file
               * name.
               */
              getSAHForPath(path) {
            ...
            /**
             * Returns a resolved Promise if the current environment has a
             * "fully-sync" SAH impl, else a rejected Promise.
             */
             ...
             * SyncAccessHandle (a.k.a. SAH) Pool VFS. It returns a Promise
             * which either resolves to a utility object described below or
             * rejects with an Error value.
             *
              ...
              return (initPromises[vfsName] = apiVersionCheck()
                .then(async function () {
                  ...
                  return thePool.isReady
                    .then(async () => {
                      /**
                       * The poolUtil object will be the result of the resolved
                       * Promise.
                       */
                      ...
                      if (sqlite3.oo1) {
                        ...
                        const OpfsSAHPoolDb = function (...args) {
                          const opt = oo1.DB.dbCtorHelper.normalizeArgs(
                            ...args,
                          );
...
*/
        ...
        if ('undefined' !== typeof Module) {
          ...
          const SABC = Object.assign(
            Object.create(null),
            {
              ...
              memory:
                Module.wasmMemory /* gets set if built with -sIMPORTED_MEMORY */,
            },
           ...
           * configuration used by a no-args call to sqlite3ApiBootstrap(), but
           * must have first loaded their WASM module in order to be able to
           * provide the necessary configuration state.
           */
   ...
   scope. */
    ...
    // end include: /build/source/ext/wasm/bld/post-js.sqlite3-vanilla.js
...
(function () {
  /**
   * In order to hide the sqlite3InitModule()'s resulting Emscripten module from
   * downstream clients (and simplify our documentation by being able to elide
   * those details), we hide that function and expose a hand-written
   * sqlite3InitModule() to return the sqlite3 object (most of the time).
   *
  ...
  /**
   * We need to add some state which our custom Module.locateFile() can see, but
   * an Emscripten limitation currently prevents us from attaching it to the
   * sqlite3InitModule function object:
   *
